...
// ================================
// IMPORTANT: Ne valider QUE si on n'affiche pas la confirmation
$showing_confirmation = false;

// Si POST (validation de commande), on vérifie d'abord
if ($_SERVER["REQUEST_METHOD"] !== "POST") {
    // GET = Affichage du formulaire → Valider la session
    require_once "session_validator.php";
}

// La session est validée - récupérer les informations
$table_id = $_SESSION['table_id'] ?? null;
$table_name = $_SESSION['table_name'] ?? 'Table inconnue';
...
if (count($cart_for_this_user) === 0 && $_SERVER["REQUEST_METHOD"] !== "POST") {
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($client_name) || empty($payment_method)) {
        ...
        // ================================
        $session_validation = $sessionManager->validateToken($session_token);
        
        if (!$session_validation['valid']) {
            // Session invalide - afficher erreur et bloquer la commande
            $error = "Erreur : Votre session a expiré. Impossible de finaliser la commande.";
        } else {
            ...
            <section class='cart-section' style='padding: 40px 15px;'>
                <div class='auto-container' style='max-width:700px; margin:auto; background:#fff; padding:25px; box-shadow:0 0 15px rgba(0,0,0,0.1); border-radius:10px;'>
                    ...
                    ?>
                        <div style='border: 1px solid #e0e0e0; padding: 15px; margin-bottom: 12px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.05); background: #fafafa;'>
                            ...
                            <div style='font-size: 0.85rem; color: #007bff; margin-top: 8px; padding-top: 8px; border-top: 1px dashed #e0e0e0;'>
                    ...
                    <div style='text-align: right; padding: 15px; border-top: 3px solid #a40301; background: #fff8f8; border-radius: 0 0 10px 10px; margin-top: 10px;'>
...
// Si on arrive ici, c'est qu'on affiche le formulaire (pas la confirmation)
?>
