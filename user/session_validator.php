 ...
 * Bloque IMMÉDIATEMENT si la table est fermée
 */
...
function validateTableSession() {
    ...
    if (!isset($_SESSION['session_last_validated'])) {
        ...
        // MODIFICATION : Valider toutes les 30 secondes (au lieu de 2 minutes)
        // Cela permet de détecter rapidement si la table a été fermée
        $time_since_validation = time() - $_SESSION['session_last_validated'];
        if ($time_since_validation > 30) { // 30 secondes
            $should_validate = true;
        }
    }
        ...
        try {
            if (!isset($link) || $link === null) {
                ...
                if ($validation['valid']) {
                    ...
                    // Marquer la session comme validée MAINTENANT
                    $_SESSION['session_last_validated'] = time();
                    
                } else {
                    // ❌ Session invalide - LA TABLE EST FERMÉE OU EXPIRÉE
                    
                    // MODIFICATION CRITIQUE : On bloque TOUJOURS si la session est invalide
                    // Peu importe si c'est la première validation ou non
                    displaySessionError(
                        "Table fermée ou session expirée", 
                        "Cette table n'est plus ouverte ou votre session a expiré. Veuillez demander au serveur de rouvrir la table et rescanner le QR code."
                    );
                    exit;
                }
            }
            ...
            // Sinon, on vérifie si ça fait trop longtemps qu'on n'a pas réussi à valider
            if (isset($_SESSION['session_last_validated'])) {
                $time_since_last_valid = time() - $_SESSION['session_last_validated'];
                
                // Si ça fait plus de 2 minutes qu'on n'a pas réussi à valider, on BLOQUE
                if ($time_since_last_valid > 120) { // 2 minutes
                    displaySessionError(
                        "Erreur de connexion", 
                        "Impossible de vérifier votre session depuis plus de 2 minutes. Veuillez rescanner le QR code."
                    );
                    exit;
                }
            }
